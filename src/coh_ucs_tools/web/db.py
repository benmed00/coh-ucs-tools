...
from coh_ucs_tools.config.paths import UPLOADS_DIR, WEB_DATA_DIR, WEB_STORAGE_DIR

logger = logging.getLogger(__name__)

DEFAULT_DB_PATH = Path(os.environ.get("SQLITE_PATH", str(WEB_DATA_DIR / "app.db")))
...
class Database:
    ...
    def _migrate_json_storage(self) -> None:
        """One-time migration from legacy JSON storage into SQLite."""
        legacy_dirs = [
            Path(__file__).parent / "storage",
            WEB_STORAGE_DIR,
        ]
        storage = next((d for d in legacy_dirs if d.exists()), None)
        if storage is None:
            return
        ...
        from coh_ucs_tools.config.paths import PROJECT_ROOT

        for index_path in (UPLOADS_DIR / "index.json", PROJECT_ROOT / "uploads" / "index.json"):
