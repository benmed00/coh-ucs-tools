...
from coh_ucs_tools.config.paths import UPLOADS_DIR
from coh_ucs_tools.tools.depot import (
    DEPOT_SPECS,
    DOWNLOADS_DIR,
...
from coh_ucs_tools.locale.coverage import build_coverage_table, coverage_to_csv, write_coverage_report
from coh_ucs_tools.profiles.game import classify_document, list_profiles as game_profile_list, validate_against_profile
from coh_ucs_tools.core.merge import PLACEHOLDER, merge_threeway_and_write
from coh_ucs_tools.io.po import export_po, export_tmx, import_po, import_tmx
from coh_ucs_tools.analysis.patch_chain import build_patch_chain
from coh_ucs_tools.io.sga import (
    extract_file,
...
from coh_ucs_tools.core.parser import serialize, write_file
from coh_ucs_tools.analysis.stats import Comparison
...
from ..deps import KNOWN_VERSIONS, _get_record, get_store, raise_profile_strict
from ..models import (
    AuditResponse,
    BatchCompareRequest,
...
from coh_ucs_tools.tools.verification import run_checklist
...
def verify_checklist_builtin() -> dict:
    from coh_ucs_tools.tools.verification import VERIFICATION_ITEMS
    return {
        "items": [
            {
                "key": i.key,
...
async def duplicate_probe_api(request: Request) -> dict:
    from coh_ucs_tools.tools.duplicate_probe import probe_instructions, write_duplicate_probe
...
def install_script(target: str = Query(...)) -> dict:
    ...
    writable_roots = [Path.home(), Path.cwd(), UPLOADS_DIR]
    is_safe = any(
...
def install_apply(body: dict) -> dict:
    ...
    writable = [Path.home(), Path.cwd(), UPLOADS_DIR]
