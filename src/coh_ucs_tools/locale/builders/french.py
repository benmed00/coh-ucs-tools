...
CoH1 was officially released in French. Place the recovered NSV French UCS at
``downloads/RelicCOH.French.NSV.ucs`` (Steam depot 4565) and run::

    python -m coh_ucs_tools.locale.builders.french
    python -m coh_ucs_tools.locale.builders.french --search-only
    python -m coh_ucs_tools.locale.builders.french --nsv path/to/French.ucs
"""
...
DEFAULT_NSV = DOWNLOADS_DIR / "RelicCOH.French.NSV.ucs"
DEFAULT_OUTPUT = Path("RelicCOH.French.complete.ucs")
REPORT_DIR = REPORTS_DIR / "french"

CONFIG = LocaleConfig(
    language="french",
    language_title="French",
    ucs_basename="RelicCOH.French.ucs",
    search_patterns=(
        "*French*", "*french*", "RelicCOH.French*.ucs", "RelicCoH.French*.ucs",
    ),
    ...
    depot_lang="french",
    web_search_note=(
        "CoH Legacy Edition French depot (SteamDB 4565) ships "
        "CoH/Engine/Locale/French/RelicCoH.French.ucs. Pull with DepotDownloader "
        "if you own Legacy Edition."
    ),
)

def search_local_french() -> list[SearchHit]:
    return search_local_locale(CONFIG)


def write_french_report(**kwargs):
    return write_locale_report(CONFIG, **kwargs)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if nsv_path is None:
        recovery_notes.append(
            f"BLOCKER: official French UCS not recovered. Place at {CONFIG.default_nsv} "
