"""Build official Polish ``RelicCOH.Polish.complete.ucs`` from recovered sources."""
...
DEFAULT_NSV = Path("downloads/RelicCOH.Polish.NSV.ucs")
DEFAULT_OUTPUT = Path("RelicCOH.Polish.complete.ucs")

CONFIG = LocaleConfig(
    language="polish",
    language_title="Polish",
    ucs_basename="RelicCOH.Polish.ucs",
    search_patterns=(
        "*Polish*", "*polish*", "RelicCOH.Polish*.ucs", "RelicCoH.Polish*.ucs",
    ),
    default_nsv=DEFAULT_NSV,
    default_output=DEFAULT_OUTPUT,
    report_dir=Path("report/polish"),
    depot_app_id=4560,
    depot_id=4568,
    depot_lang="polish",
    web_search_note=(
        "CoH Legacy Edition Polish depot (SteamDB app 4560, depot 4568 — verify on SteamDB) "
        "ships CoH/Engine/Locale/Polish/RelicCoH.Polish.ucs."
    ),
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if nsv_path is None:
        recovery_notes.append(
            f"BLOCKER: official Polish UCS not recovered. Place at {CONFIG.default_nsv} "
