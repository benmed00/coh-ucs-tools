"""Build official Italian ``RelicCOH.Italian.complete.ucs`` from recovered sources."""
...
DEFAULT_NSV = Path("downloads/RelicCOH.Italian.NSV.ucs")
DEFAULT_OUTPUT = Path("RelicCOH.Italian.complete.ucs")

CONFIG = LocaleConfig(
    language="italian",
    language_title="Italian",
    ucs_basename="RelicCOH.Italian.ucs",
    search_patterns=(
        "*Italian*", "*italian*", "RelicCOH.Italian*.ucs", "RelicCoH.Italian*.ucs",
    ),
    default_nsv=DEFAULT_NSV,
    default_output=DEFAULT_OUTPUT,
    report_dir=Path("report/italian"),
    depot_app_id=4560,
    depot_id=4567,
    depot_lang="italian",
    web_search_note=(
        "CoH Legacy Edition Italian depot (SteamDB app 4560, depot 4567 — verify on SteamDB) "
        "ships CoH/Engine/Locale/Italian/RelicCoH.Italian.ucs."
    ),
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if nsv_path is None:
        recovery_notes.append(
            f"BLOCKER: official Italian UCS not recovered. Place at {CONFIG.default_nsv} "
            f"(Steam depot {CONFIG.depot_id}) and re-run."
        )
        write_locale_report(
            CONFIG, search_hits=search_hits, nsv_path=None, thq_path=thq_path,
            russian_path=args.russian, english_complete_path=args.english_complete,
            output_path=args.output if args.output.is_file() else None,
            build_info=None, recovery_notes=recovery_notes,
        )
        print("\n".join(recovery_notes))
        return 1
    ...
    if args.search_only:
        write_locale_report(
            CONFIG, search_hits=search_hits, nsv_path=nsv_path, thq_path=thq_path,
            russian_path=args.russian, english_complete_path=args.english_complete,
            output_path=None, build_info={"search_only": True}, recovery_notes=recovery_notes,
        )
        return 0
    ...
    if not args.report_only:
        ...
        if build_info["russian_gap_placeholders"]:
            print(f"WARNING: {build_info['russian_gap_placeholders']} gaps filled with {PLACEHOLDER}.")

    write_locale_report(
        CONFIG, search_hits=search_hits, nsv_path=nsv_path, thq_path=thq_path,
        russian_path=args.russian, english_complete_path=args.english_complete,
        output_path=args.output if args.output.is_file() else None,
        build_info=build_info, recovery_notes=recovery_notes,
    )
    return 0
