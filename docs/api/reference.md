...
1. the **Python API** — the `coh_ucs_tools` package (`core`, `analysis`, `locale`, `tools`), and
2. the **REST API** — the FastAPI application in `coh_ucs_tools.web`, self-documented
...
```python
from coh_ucs_tools.core.parser import parse_file
from coh_ucs_tools.analysis.stats import Comparison, generate_report
