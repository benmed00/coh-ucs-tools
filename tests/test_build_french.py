...
from build_french import SearchHit, build_complete, pick_nsv_path, search_local_french, write_french_report
...
class BuildFrenchTests(unittest.TestCase):
    ...
    def test_write_french_report_creates_statistics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            import build_french as bf

            original = bf.REPORT_DIR
            try:
                bf.REPORT_DIR = report_dir
                write_french_report(
                    search_hits=[],
                    nsv_path=fr,
                    thq_path=None,
