...
from parser import BOM_LE, iter_ucs_lines_chunked, parse_file, parse_file_chunked, scan_file_stats
from depot_downloader import DEPOT_SPECS, download_depot_locale, list_depot_specs, run_locale_build
...
class DepotDownloaderTests(unittest.TestCase):
    ...
    @patch("depot_downloader.subprocess.run")
    @patch("depot_downloader.find_depotdownloader")
    def test_download_copies_largest_ucs(self, mock_find: MagicMock, mock_run: MagicMock) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                with patch("depot_downloader.DOWNLOADS_DIR", downloads):
...
class LocaleBuildModuleTests(unittest.TestCase):
    def test_polish_italian_configs(self) -> None:
        from build_polish import CONFIG as pl
        from build_italian import CONFIG as it
