"""Build official Spanish ``RelicCOH.Spanish.complete.ucs`` from recovered sources.

CoH1 was officially released in Spanish. Place the recovered NSV Spanish UCS at
``downloads/RelicCOH.Spanish.NSV.ucs`` (Steam depot 4566) and run::

    python build_spanish.py
    python build_spanish.py --search-only
    python build_spanish.py --nsv path/to/Spanish.ucs
"""
...
DEFAULT_NSV = Path("downloads/RelicCOH.Spanish.NSV.ucs")
DEFAULT_OUTPUT = Path("RelicCOH.Spanish.complete.ucs")

CONFIG = LocaleConfig(
    language="spanish",
    language_title="Spanish",
    ucs_basename="RelicCOH.Spanish.ucs",
    search_patterns=(
        "*Spanish*", "*spanish*", "RelicCOH.Spanish*.ucs", "RelicCoH.Spanish*.ucs",
    ),
    default_nsv=DEFAULT_NSV,
    default_output=DEFAULT_OUTPUT,
    report_dir=Path("report/spanish"),
    depot_app_id=4560,
    depot_id=4566,
    depot_lang="spanish",
    web_search_note=(
        "CoH Legacy Edition Spanish depot (SteamDB 4566) ships "
        "CoH/Engine/Locale/Spanish/RelicCoH.Spanish.ucs. Pull with DepotDownloader "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if official:
        print("Spanish UCS candidate(s):")
        for h in official:
            print(f"  {h.path} ({h.note})")
    else:
        print("No Spanish RelicCOH UCS found in local installs or downloads/.")
    ...
    if nsv_path is None:
        recovery_notes.append(
            f"BLOCKER: official Spanish UCS not recovered. Place at {CONFIG.default_nsv} "
